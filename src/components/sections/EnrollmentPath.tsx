    ...
    <Section id="enrollment" className="bg-[var(--tone-paper)]">
      <ClipReveal>
        ...
        <p className="mt-3 max-w-2xl text-sm text-[var(--tone-muted)]">
          Admission depends on classroom availability. Waitlists are common — we keep families
        ...
        {business.enrollment.steps.map((step, i) => (
          <ClipReveal key={step.title}>
            <li className="flex gap-4 rounded-xl border border-black/10 bg-white p-5">
              <span
                className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[var(--tone-accent)] text-sm font-bold text-white"
                aria-hidden
              >
                {i + 1}
              </span>
